    ...
    public sealed class StorageBackendDemoForm : Form
    {
        ...
        public StorageBackendDemoForm()
        {
            ...
            Label hintLabel = new Label
            {
                ...
                Text = "文本和 CSV 后端会落盘到本地文件。模拟数据库后端会把数据写进右侧表格预览，用来说明如何替换成真实数据库实现。"
            };
            ...
            splitContainer.Panel2.Controls.Add(_previewGroup);
        ...
        private void RecreateLogger()
        {
            ...
            ILoggerFactory factory = CreateFactory(backendKind);
        ...
        private ILoggerFactory CreateFactory(StorageBackendKind backendKind)
        {
            ...
            switch (backendKind)
            {
                ...
                case StorageBackendKind.SimulatedDatabase:
                    ...
                    ILoggerFactory dbFactory = new LogStoreLoggerFactory(tableFactory);
                    _tableBackend = tableFactory.CurrentBackend;
                    return new DeferredBackendFactory(dbFactory, tableFactory, backend => _tableBackend = backend);
                default:
                    _tableBackend = null;
                    return new LogStoreLoggerFactory(
        ...
        private sealed class DeferredBackendFactory : ILoggerFactory
        {
            private readonly ILoggerFactory _innerFactory;
            private readonly DemoTableLogStorageBackendFactory _backendFactory;
            private readonly Action<DemoTableLogStorageBackend> _onLoggerCreated;

            public DeferredBackendFactory(
                ILoggerFactory innerFactory,
                DemoTableLogStorageBackendFactory backendFactory,
                Action<DemoTableLogStorageBackend> onLoggerCreated)
            {
                _innerFactory = innerFactory;
                _backendFactory = backendFactory;
                _onLoggerCreated = onLoggerCreated;
            }

            public ILoggerOutput CreateLogger(string name)
            {
                ILoggerOutput logger = _innerFactory.CreateLogger(name);
                _onLoggerCreated?.Invoke(_backendFactory.CurrentBackend);
                return logger;
            }
        }
    }
